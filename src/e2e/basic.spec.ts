...
test('basic navigation and presence of content', async ({ page }) => {
  ...
  await expect(page.getByText('Master GATE CSE')).toBeVisible();

  // Check roadmap preview
  await page.goto('/roadmap');
  await expect(page.getByText('GATE Roadmap Preview')).toBeVisible();
  await expect(page.getByText('Digital Logic')).toBeVisible();
  await expect(page.getByText('Boolean Algebra')).toBeVisible();
});
