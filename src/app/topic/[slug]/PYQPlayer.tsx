...
export default function PYQPlayer({ pyqs, topicSlug }: { pyqs: any[], topicSlug: string }) {
  ...
  const getAiHelp = async () => {
    ...
    try {
        ...
        if (resp.status === 429) {
          setAiError(data.error || "Daily limit reached. Please try again tomorrow.");
          return;
        }
        ...
        console.error(e);
        setAiError("Connection error. Please check your network.");
    } finally {
         ...
         {(aiExplanation || aiError) && (
           ...
           >
              ...
              <div className="prose prose-invert max-w-none font-medium leading-relaxed text-slate-300 text-sm md:text-base">
                 {aiError ? (
                   <p className="text-red-200 font-bold">{aiError}</p>
                 ) : (
