...
export default function PYQPlayer({ pyqs, topicSlug }: { pyqs: any[], topicSlug: string }) {
  ...
  const getAiHelp = async () => {
    ...
    try {
        ...
        if (resp.status === 429) {
            setAiError(data.error || "Daily AI limit reached. Please try again tomorrow.");
            return;
        }
      ...
      <AnimatePresence>
         {(aiExplanation || aiError) && (
           <motion.div
             ...
             exit={{ opacity: 0, y: 20 }}
             className={cn(
                "premium-card p-6 md:p-10 bg-slate-900 text-white",
                aiError ? "border-red-500/30" : "border-brand-500/30"
             )}
           >
              <div className="flex items-center gap-3 mb-6 md:mb-8">
                 <div className={cn(
                    "w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center text-white shrink-0",
                    aiError ? "bg-red-500" : "bg-brand-500"
                 )}>
                    <BrainCircuit className="w-5 h-5 md:w-6 md:h-6" />
                 </div>
                 <div>
                    <h3 className="text-lg md:text-xl font-black">
                        {aiError ? "Limit Reached" : "Technical Derivation"}
                    </h3>
                    <p className={cn(
                        "text-[10px] font-bold uppercase tracking-widest",
                        aiError ? "text-red-400" : "text-brand-400"
                    )}>
                        {aiError ? "Policy Enforcement" : "Gemini 1.5 Pro Analysis"}
                    </p>
                 </div>
                 <button onClick={() => { setAiExplanation(null); setAiError(null); }} className="ml-auto text-slate-500 hover:text-white">
              ...
              {aiError ? (
                <div className="flex flex-col items-center justify-center py-6 text-center">
                    ...
                    <p className="text-slate-300 font-medium mb-2">{aiError}</p>
                    <p className="text-xs text-slate-500">Upgrade to Pro for unlimited AI-powered technical derivations.</p>
                </div>
              ) : (
