...
export async function POST(req: Request) {
  ...
  const rateLimit = await RateLimitService.checkLimit(session.user.id, 'ai_explain', 20, 24);
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to generate explanation" }, { status: 500, headers });
  }
}
