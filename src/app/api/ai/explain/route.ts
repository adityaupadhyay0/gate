...
export async function POST(req: Request) {
  ...
  const rateLimit = await RateLimitService.checkLimit(session.user.id, "ai_explain", 20);

  const headers = {
    "X-RateLimit-Limit": rateLimit.limit.toString(),
    "X-RateLimit-Remaining": rateLimit.remaining.toString(),
    "X-RateLimit-Reset": Math.ceil(rateLimit.reset.getTime() / 1000).toString(),
  };

  if (!rateLimit.isAllowed) {
    return NextResponse.json(
      {
        error: "Rate limit exceeded. You have used your 20 free explanations for the next 24 hours.",
