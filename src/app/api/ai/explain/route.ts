...
export async function POST(req: Request) {
  ...
  const rateLimit = await RateLimitService.checkLimit(session.user.id, 'ai-explain', 20, 24);
  ...
  if (!rateLimit.success) {
    return NextResponse.json(
      { error: "Daily AI explanation limit reached. Use 'Instant Insight' or try again later." },
      { status: 429, headers }
    );
  }
