...
export async function POST(req: Request) {
  ...
  const rateLimitStatus = await RateLimitService.checkAndIncrement(
    session.user.id,
    "ai_explain",
    20,
    24
  ...
  const headers = {
    ...
    "X-RateLimit-Reset": Math.floor(rateLimitStatus.resetAt.getTime() / 1000).toString(),
  };

  if (rateLimitStatus.isLimited) {
    return NextResponse.json(
      { error: "Rate limit exceeded. Try again in a few hours." },
      { status: 429, headers }
    );
  }
