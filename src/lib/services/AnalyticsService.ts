...
export class AnalyticsService {
  static async getOverallStats(userId: string): Promise<DashboardStats> {
    ...
    try {
      diagnosticData = user?.diagnosticResult;
    ...
    const benchmark = await PeerBenchmarkingService.getBenchmark(userId, progress, diagnosticData);
    ...
    return {
      ...
      rankEstimation: `Rank #${benchmark.rank}`,
      percentile: benchmark.percentile,
      totalUsers: benchmark.totalUsers,
      isCalibrated,
      calibrationProgress
    };
  }
