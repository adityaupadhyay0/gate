...
## Goal
Implement a global ranking system that estimates a student's potential AIR based on their diagnostic performance and syllabus mastery compared to the peer group.

## Why It Matters
GATE is a competitive exam where relative performance is everything. Providing students with a realistic, dynamic rank estimation motivates consistent study and helps them identify how far they are from their target IIT/IISc cutoff.

## Scope
- Peer data aggregation engine.
- Dynamic percentile calculation logic.
- Rank estimation heuristic based on mastery and accuracy.
- Dashboard 'Global Standing' card.

## Dependencies
- `AnalyticsService` for mastery data.
- Aggregate Prisma queries.

## Implementation Plan
1. Define the rank estimation formula: (Diagnostic Avg * 0.4) + (Syllabus Mastery * 0.6).
2. Create `BenchmarkingService` to calculate peer averages.
3. Update User model to cache `estimatedRank`.
4. Implement the 'Global Standing' UI component on the dashboard.
5. Verify with simulated peer data.

## UX Improvements
- High-impact visualization of current rank vs. target.
- "Top X% of aspirants" motivator badge.

## Validation Strategy
- Data simulation tests for rank distribution.
- UI responsiveness audit.

## Risks
- Rank volatility with small user bases (mitigated by normalization against historical GATE cutoffs).

## Rollback Strategy
- Hide the rank component if data is statistically insignificant.

## Completion Criteria
- User sees a dynamic rank estimation on their dashboard.
- Percentile standing is updated in real-time.
...
- **Security Concerns**: None. Rate-limiting active for AI endpoints.
...
- **Rate Limiting**: Window-based enforcement (20 req/24h) active.
...
- **Rate Limiting**: Active (20 req/24h) for `/api/ai/explain`.
...
- Implemented `RateLimitService` with atomic Prisma updates.
- Integrated headers (`X-RateLimit-*`) and 429 error handling in UI.
- Verified via unit tests (19/19 passing).

### Architecture Changes
- New `RateLimit` model added to Prisma.
- Centralized rate limiting logic in `RateLimitService`.

### UX Findings
- Handling 429 errors in `PYQPlayer` prevents UI 'silent failures' and guides users to the precomputed 'Instant Insight'.
