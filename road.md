...
## Priority
Medium (Motivation & Analytics)

## Goal
Estimate a user's global GATE rank by benchmarking their performance against the cohort and historical data.

## Why It Matters
Rank is the ultimate metric for GATE. Providing a real-time estimate motivates users and helps them understand where they stand in the competitive landscape.

## Scope
- Global average calculation for diagnostic and topic mastery.
- Percentile-based rank estimation algorithm.
- Dashboard "Estimated Global Rank" widget.

## Implementation Plan
1. Research rank estimation heuristics.
2. Implement benchmarking aggregation in `AnalyticsService`.
3. Create Rank Estimation UI component.
...
- Rate Limiting for AI Routes (Cost control)
...
## [2025-05-16 02:15:00]
### Completed
- Implemented `RateLimit` Prisma model and synchronized database.
- Created `RateLimitService` for centralized, window-based limit tracking.
- Integrated rate limiting into `/api/ai/explain` with `X-RateLimit-*` headers.
- Enhanced `PYQPlayer` to handle and display rate limit errors.
- Fixed JSX parsing error in `DiagnosticTestClient.tsx` that blocked production builds.
- Verified all systems with 20/20 passing unit tests.

### Architecture Changes
- Added database-backed rate limiting layer for AI features.
- Standardized AI API response headers for client-side budget tracking.

### UX Findings
- Returning the specific error message from the API in the `PYQPlayer` maintain context and reduces user confusion when limits are reached.

### Next Recommended Actions
- Implement Peer Benchmarking System (Global rank estimation) to provide comparative analytics.
...
- Explicit academic states for rating buttons improve clarity on which rating is being persisted.
